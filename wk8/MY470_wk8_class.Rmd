...
When you are in a code chunk, using the keyboard shortcuts to run code will only run the line you have currently selected.if you want the whole code chunk, shift+ctrl+enter
...
```{r}
list <- seq(1:100)
filter_3x <- sapply(list, function(x) {x%% 3!=0})
list_no_3x <- list[filter_3x]
...
```{r}
lst2 <- c(1, 3, 2, 4.5, 7, 8, 10, 3, 5, 4, 7, 3.33)
...
``` {r}
zero_vector <- function(a) {
  new_vector <- rep(0L, length(a))
  return(new_vector)
}

lst = c(1, 2, 3)
zerolst <- zero_vector(lst)
print(zerolst)
```
_Your cell here._
...
```{r}
scientists <- list('mathematician', 'physicist',
              'chemist',  'biologist',
              'mathematician', 'physicist')

names(scientists) <- c('Alan Turing', 'Richard Feynman',
              'Marie Curie', 'Charles Darwin',
              'Ada Lovelace', 'Werner Heisenberg')

print(scientists)

print_profs <- function(named_list) {
  for (i in names(named_list)) {
    print(paste0(i , "was a " , named_list[i] , "."))
  }
}

print_profs(scientists)

paste0(names(scientists), 'was a ', scientists, '.')
